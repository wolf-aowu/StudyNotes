...
<font color = orange>注意：</font> 部分图片需要采用 RGBA Compressed ETC2 8 bit：大小为原图 1/4  ，否则图片会与原图相差很大。

- RGBA Crunched ETC2 是一种有损压缩。<font color = #8c7ae6>在我使用后，想重新设置成 RGB + 1 - bit Alpha Compressed ETC2 4 bit 就报错了。后面再看吧。</font>

  <font color = #e84118>报的错：</font>Failed to launch Tools/etccompress.exe
...
如果 Max Size 比图片的长 和 宽都大，那么什么也不会发生。

如果 Max Size 比图片的长 和 宽任意一个小，那么会将图片的长 和 宽中最大的那个数值会切成 Max Size，剩下那个数按之前比例变小取整。
...
经测试对比，有变化但变化不大，因此不是特别精细锐利的图片直接使用 Mitchell 即可。
...
```c#
    ...
    /// 获取目标压缩格式
    /// </summary>
    /// <param name="platform">选择的目标平台</param>
    /// <param name="width">纹理本身的长</param>
    /// <param name="height">纹理本身的宽</param>
    /// <param name="haveAlpha">纹理是否有透明通道</param>
    /// <returns>目标压缩格式</returns>
    

        //分几种情况讨论，是不是 2 的幂以及是不是有alpha通道
    ...
    #region 判断一个数是否是 2 的幂
    ///// <summary>
    ///// 判断一个数是否是 2 的幂
    ///// 用二进制与运算，如果一个数是 2 的幂，n & (n-1) = 0
    ///// </summary>
    ...
    /// 用反射获得纹理的宽和高
    /// </summary>
    /// <param name="textImporter"></param>
    /// <param name="width"></param>
    /// <param name="height"></param>
    
        // 因为 GetWidthAndHeight 方法的访问修饰符为 internal 所以需要这样获取纹理的长 和 宽
        // BindingFlags.Instance 指定实例成员将被包括在搜索中
        // BindingFlags.NonPublic 指定非公开成员被包括在搜索中
